...
func TestBasinProvider_Upload(t *testing.T) {
	...
	filedata1 := []byte{'H', 'e', 'l', 'l', 'o'}
	{
		...
		err := client.Upload(
			context.Background(), "test", "test", uint64(5), buf, app.NewSigner(privateKey), bytes.NewBuffer([]byte{}), 1,
		)
		require.NoError(t, err)
	...
	filedata2 := []byte{'W', 'o', 'r', 'l', 'd'}
	{
		...
		err := client.Upload(context.Background(), "test2", "test2", uint64(5), buf, app.NewSigner(privateKey), bytes.NewBuffer([]byte{}), 1) // nolint
		require.NoError(t, err)
	...
	filedata3 := []byte{'W', 'O', 'R', 'L', 'D'}
	{
		...
		err := client.Upload(context.Background(), "test", "test", uint64(5), buf, app.NewSigner(privateKey), bytes.NewBuffer([]byte{}), 1) // nolint
		require.NoError(t, err)
